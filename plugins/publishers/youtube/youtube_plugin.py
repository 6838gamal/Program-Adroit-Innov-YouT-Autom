...
class YouTubePublisherPlugin(PublisherPort):
    ...
    def get_authorization_url(self, redirect_uri: str) -> tuple[str, str]:
        ...
        try:
            ...
            if verifier:
                ...
                logger.info("💾 Stored PKCE verifier for state=%s…", state[:8])
            else:
                logger.warning("⚠️ No code_verifier generated by Flow")
        except Exception as e:
            logger.warning("Could not capture code_verifier: %s", e)
    ...
    def exchange_code(
        ...
        if state:
            ...
            if verifier:
                ...
                logger.info("🔑 Using stored PKCE verifier for state=%s…", state[:8])
            else:
                logger.warning("⚠️ No PKCE verifier found for state=%s…", state[:8])
    ...
    def _get_channel_info(self, credentials) -> dict:
        """Fetch full channel info from YouTube Data API v3."""
        try:
            ...
            return {
                ...
                "channel_description": snippet.get("description", "") or "",
                "channel_country":     snippet.get("country", "") or "",
    ...
    def _get_user_info(self, credentials) -> dict:
        """Fetch Google account email/name via userinfo endpoint."""
        try:
            from googleapiclient.discovery import build
    ...
    async def authenticate(self, credentials: dict) -> AuthResult:
        ...
        try:
            ...
            # اجلب معلومات القناة ومعلومات حساب Google بالتوازي
            channel_info, user_info = await asyncio.gather(
                loop.run_in_executor(None, self._get_channel_info, creds_obj),
                loop.run_in_executor(None, self._get_user_info, creds_obj),
        ...
        # دمج معلومات المستخدم مع معلومات القناة
        # (القناة لها الأولوية في حالة تعارض المفاتيح)
        merged = {**user_info, **channel_info}
